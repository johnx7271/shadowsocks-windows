...
namespace Shadowsocks.View
{
	public class MenuViewController
    {
        ...
        public MenuViewController(ShadowsocksController controller)
        {
			...
			LoadCurrentConfiguration(config);
		...
		/// <summary>
		/// config is not changed inside the call
		/// </summary>
        ...
        {
            Configuration config = LoadCurrentConfiguration();
            UpdateTrayIcon(config);
        }
        ...
        private Configuration LoadCurrentConfiguration(Configuration config = null)
        {
			if(config == null)
				config = controller.GetConfigurationCopy();
            UpdateServersMenu(config);
            ...
            UpdateUpdateMenu(config);
			return config;
        }

		/// <summary>
		/// config is not changed in the call.
		/// </summary>		
        private void UpdateServersMenu(Configuration config)
        {		
            ...
            {
                if (item.Tag != null && (item.Tag.ToString() == config.index.ToString() 
					|| item.Tag.ToString() == config.strategy))
                {
                    item.Checked = true;
                }
            }
		...
		/// config is not changed inside the call.
		/// </summary>
        private void UpdateUpdateMenu(Configuration config)
        {            
            autoCheckUpdatesToggleItem.Checked = config.autoCheckUpdate;
            checkPreReleaseToggleItem.Checked = config.checkPreRelease;
        }
        ...
        private void autoCheckUpdatesToggleItem_Click(object sender, EventArgs e)
        {
            ...
            UpdateUpdateMenu(configuration);
        }
